...
fetchLoginResponse(context, credentials) async {
  await http.post(Res.loginAPI as Uri, body: credentials).then((response) {
...
Future<SignUpResponse> fetchSignUpResponse(data) async {
  ...
  final response = await http.post(Res.registerAPI as Uri, body: body);
  if (response.statusCode == 200) {
    Map userMap = json.decode(response.body);
...
Future<List<AccountType>> getAllAccountType() async {
  var res = await http.get(
    Uri.encodeFull(Res.accountType) as Uri,
  );
